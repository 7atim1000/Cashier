...
const Invoices = () => {
    ...
    return (
        <section className ='flex h-[calc(100vh)] overflow-y-scroll scrollbar-hidden gap-3 bg-[#1a1a1a]'>

            <div className ='flex-[2] h-[calc(100vh)] overflow-y-scroll scrollbar-hidden '>
               
                <div className ='flex items-center justify-between p-1 bg-[#383838]'>
                    
                    <div className ='flex items-center justify-between'>
                        <p className ='p-2 text-sky-400 border-b border-[#e6e6e6]'>
                    ...
                    <div className='flex items-center justify-between'>
                        ...
                        <p className='p-2 text-sky-400 border-b border-[#e6e6e6] '>{allInvoices.reduce((acc, invoice) => acc + invoice.bills.totalWithTax, 0).toFixed(2)}
                            <span className='text-xs font-normal text-[#e6e6e6]'> AED</span>
                        </p>
                    </div>
                ...
                <hr className ='border-t border-[#1f1f1f] m-1'/>


                <div className='h-[calc(100vh)] flex  flex-col items-start gap-4 justify-start p-2 px-2 bg-[#1a1a1a] overflow-y-scroll scrollbar-hidden' >
            ...
            <div className ='flex-[4] h-[calc(100vh)] overflow-y-scroll scrollbar-hidden bg-[#1f1f1f]'>
                
                <div className='flex items-center justify-between px-10 py-2 bg-[#383838]'>
                    ...
                    <div className='flex gap-2 mt-1'>
                        ...
                        <button className={`${frequency === '30' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                            onClick={() => setFrequency('30')}
                        >One Month
                        </button>
                        <button className={`${frequency === '365' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                            onClick={() => setFrequency('365')}
                        >One Year
                        </button>


                        <button className={`${orderStatus === 'In Progress' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                            onClick={() => setOrderStatus('In Progress')}
                        >In Progress
                        </button>
                        <button className={`${orderStatus === 'Completed' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                            onClick={() => setOrderStatus('Completed')}
                        >Completed
                        </button>


                        <button className={`${shift === 'Morning' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                            onClick={() => setShift('Morning')}
                        >Morning
                        </button>

                        <button className={`${shift === 'Evening' ? 'bg-[#383838] text-[#f6b100]' : 'bg-[#1a1a1a]'} p-2 rounded-lg  shadow-lg/30 text-xs text-[#e6e6e6] font-medium cursor-pointer`}
                    ...
                    <div className='overflow-x-auto px-5'>
                        <table className='w-full text-left'>
                            <thead className='bg-[#1a1a1a] text-xs font-semibold text-sky-400'>
                                <tr className ='border-b-2  border-[#e6b100]'>
                            ...
                            <tbody>
                                {
                                    allInvoices.length === 0
                                        ? (<p className='p-1 w-50 text-sm text-[#be3e3f] flex justify-center'>Invoices menu is empty!</p>)
