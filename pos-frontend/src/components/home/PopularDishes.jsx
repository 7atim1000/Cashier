...
const  PopularDishes = () => {
    ...
    return (
       <div className='' >
            <div className='bg-[#1a1a1a] w-full rounded-lg items-center h-[calc(109vh)] overflow-y-scroll scrollbar-hidden'>
                
                <div className='flex justify-between items-center px-6 py-1 w-[100%]'>
                    <h1 className='text-sky-500 text-md   font-semibold tracking-wide'>Popular Dishes</h1>
                ...
                <div className= ''>
                    {
                        services.map((dish) => {
                            return (
                                <div key={dish.id} className= 'flex items-center gap-4 bg-[#1f1f1f] rounded-[5px] px-6 py-1 mx-6 mt-2'>
                                    {/* <h1 className='text-[#02ca3a] font-bold text-sm mr-4' >{dish._id < 10 ? `0${dish._id}` : dish._id}</h1> */}
                                    <img src={dish.image} alt={dish.name} className='w-[40px] h-[40px] rounded-full border-b-3 border-[#0ea5e9]' />
                                    <div>
