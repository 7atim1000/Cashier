...
const OrderList = ({key, order}) => {
    ...
    return (
        ...
        <div className='flex items-center gap-5 mb-3 bg-[#1f1f1f] border-b p-2 border-sky-400 rounded-lg'>
            ...
            <button className='bg-[#f6b100] p-1 w-10 h-10 text-xs font-semibold rounded-full '>{getAvatarName(order.customerDetails.name)}</button>
                ...
                <div className='flex flex-col items-center gap-2'>
                    {
                        order.orderStatus === "Completed" ? (
                            <>
                                <p className='text-green-600 bg-[#2e4a40] px-2 rounded-lg text-sm'><FaCheckDouble className='inline mr-2' />{order.orderStatus}</p>
                                <p className='text-[#ababab] text-xs'><FaCircle className='inline mr-1 text-green-600' />Ready to serve</p>
                            </>
